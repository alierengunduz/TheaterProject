const TheatreFilter = require("../models/theatreFilter");

//get all theatre filters
const getTheatreFilters = async (req, res) => {
  try {
    const theatreFilters = await TheatreFilter.find({});
...
const getTheatreFilterById = async (req, res) => {
  try {
    const theatreFilter = await TheatreFilter.findById(req.params.id);
...
const createTheatreFilter = async (req, res) => {
  ...
  try {
    const theatreFilter = new TheatreFilter({
      name,
    });
...
const deleteTheatreFilter = async (req, res) => {
  try {
    const { id } = req.params;
    const theatreFilter = await TheatreFilter.findOneAndDelete({ _id: id });
