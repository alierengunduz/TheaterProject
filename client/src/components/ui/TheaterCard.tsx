...
const TheaterCard: FC<ChildrenCardProps> = ({ item }) => {
  const navigate = useNavigate();

  return (
    <div
      onClick={() => navigate(`/theater/${item._id}`)}
      className="relative lg:w-[300px] md:w-[350px]  sm:w-[300px] w-full  h-[440px] group overflow-hidden rounded-lg shadow-lg hover:shadow-2xl transition-shadow duration-300 cursor-pointer"
    >
      {/* Resim (Üst Kısım) */}
