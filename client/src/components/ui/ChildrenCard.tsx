...
const ChildrenCard: FC<ChildrenCardProps> = ({ item }) => {
  const navigate = useNavigate();

  return (
    <div
      onClick={() => navigate(`/children/${item._id}`)}
      className="relative sm:w-[280px] w-full h-[450px] group overflow-hidden rounded-lg shadow-lg hover:shadow-2xl transition-shadow duration-300 cursor-pointer"
    >
      {/* Resim (Üst Kısım) */}
      <div className="w-full h-2/4 bg-gradient-to-br from-red-600 to-orange-300">
        <img
          src={
            item.image[0]
      ...
      {/* İçerik (Alt Kısım) */}
      <div className="p-4 bg-white h-2/4 rounded-b-lg flex flex-col justify-between">
