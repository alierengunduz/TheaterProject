...
const ActorCard: FC<ChildrenCardProps> = ({ item }) => {
  const navigate = useNavigate();

  return (
    <div
      onClick={() => navigate(`/actor/${item._id}`)}
      className="relative w-[320px] h-[440px] group overflow-hidden rounded-lg shadow-lg hover:shadow-2xl transition-shadow duration-300 cursor-pointer"
    >
      {/* Resim (Üst Kısım) */}
