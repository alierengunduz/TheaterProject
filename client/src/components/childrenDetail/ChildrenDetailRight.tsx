...
const ChildrenDetailRight = () => {
  const { childrenDetail, status, error } = useSelector(
  ...
  const formattedTime = childrenDetail?.time
    ? new Date(childrenDetail.time).toLocaleTimeString()
    : "";
...
export default ChildrenDetailRight;
